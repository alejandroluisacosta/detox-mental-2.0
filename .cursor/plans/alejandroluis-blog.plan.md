...
- **Spanish public copy in the page layer** (same as `/theory`). Code, comments, prompts, schema, and API names stay English. Do not add blog strings to journal/education locale files.
...
| `/alejandroluis/blog` | public | Chronological list of published posts (title, date, excerpt) |
...
Landing copy (draft, Spanish): title **Alejandro Luis**, one short line in your voice, button **BLOG**. No Detox Mental wordmark on this screen (the domain already carries the product; this surface is personal).
...
└── parseBlogPost.js          # slug, title, excerpt, body, status
```
...
Index `published_at DESC` for the public list (`WHERE status = 'published'`). Reuse `update_updated_at_column()`. Document in [`backend/src/db/README.md`](backend/src/db/README.md).
...
| --- | --- | --- | --- |
| GET | `/blog/posts` | public | Published only: `{ id, slug, title, excerpt, publishedAt }` newest first |
...
**Validation** in `parseBlogPost.js` (tested): trim title; slug lowercase hyphenated `[a-z0-9-]+`, length cap; body non-empty; status `draft | published`; excerpt optional, cap ~500 chars. Duplicate slug → 409.
...
`frontend/src/utils/blogSlug.js` — client slug suggestion from title (same rules as the server). Tests colocated.

`apiFetch` for all blog requests (cookie sent automatically when you are signed in). Public GETs still work anonymous.

**Composer fields:** title, slug (auto from title until edited), excerpt, markdown body, status (draft / published), save. Success → `/alejandroluis/blog/:slug`. Keyboard: primary save button is a real `<button type="submit">`.

**Empty index:** a short Spanish line that there are no articles yet. No CTA into Detox Mental.

**Loading / error:** follow journal list patterns (status text, retry where a fetch failed).
